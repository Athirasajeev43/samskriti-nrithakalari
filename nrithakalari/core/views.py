...
def register(request):
    ...
    if request.method == "POST":
        ...
        if user is None:
            # First-time registration: create both the User and UserProfile.
            user = User.objects.create_user(username=username, password=password)
            UserProfile.objects.create(user=user, age=age_int, phone=phone)
        else:
            # Username exists but profile is missing (e.g. superuser created before refactor).
...
def payment(request):
    ...
    if profile.admission_fee_paid:
        ...
        return redirect("dashboard_course_details")
    ...
    if request.method == "POST":
        ...
        # We create the MonthlyFeePayment entry so Online Classes can be joined.
...
def profile(request):
    ...
    - Allow students to edit their profile (age/phone/address/photo).
    - Show selected course/timetable + payment status.
    """
    ...
    if request.method == "POST":
        ...
        profile_obj.age = age_int
        profile_obj.phone = phone
        profile_obj.address = address

        profile_obj.save()

        # Update Django User name/password (students log in using User.username).
        if name:
        ...
        if new_password:
            ...
            # Basic validation for mini-project usability.
